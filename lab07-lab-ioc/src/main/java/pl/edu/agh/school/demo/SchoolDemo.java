...
//        school = new School(new SchoolDAO(new SerializablePersistenceManager()));
    ...
    public void initClass() throws ParseException {
        if (school.findClass("1A", "humane").isEmpty()) {
            ...
            subject.addTerm(new Term(DayOfWeek.MONDAY, timeFormat.parse("10:30"), 45));
            subject.addTerm(new Term(DayOfWeek.THURSDAY, timeFormat.parse("12:50"), 45));
            subject.setTeacher((Teacher) school.findPerson("Thomas", "Anderson").iterator().next());
            schoolClass.addSubject(subject);

            subject = injector.getInstance(Subject.class);
            subject.setName("English");
            subject.addTerm(new Term(DayOfWeek.MONDAY, timeFormat.parse("12:00"), 45));
            subject.addTerm(new Term(DayOfWeek.TUESDAY, timeFormat.parse("11:00"), 45));
            subject.addTerm(new Term(DayOfWeek.WEDNESDAY, timeFormat.parse("10:30"), 45));
            subject.addTerm(new Term(DayOfWeek.THURSDAY, timeFormat.parse("11:00"), 45));
            subject.setTeacher((Teacher) school.findPerson("Han", "Solo").iterator().next());
            schoolClass.addSubject(subject);

            subject = injector.getInstance(Subject.class);
            subject.setName("Computer Science");
            subject.addTerm(new Term(DayOfWeek.TUESDAY, timeFormat.parse("09:00"), 45));
            subject.setTeacher((Teacher) school.findPerson("Thomas", "Anderson").iterator().next());
            schoolClass.addSubject(subject);

            subject = injector.getInstance(Subject.class);
            subject.setName("Chemistry");
            subject.addTerm(new Term(DayOfWeek.WEDNESDAY, timeFormat.parse("11:30"), 45));
            subject.setTeacher((Teacher) school.findPerson("Princess", "Leia").iterator().next());
            schoolClass.addSubject(subject);

            subject = injector.getInstance(Subject.class);
            subject.setName("Potions");
            subject.addTerm(new Term(DayOfWeek.FRIDAY, timeFormat.parse("09:00"), 90));
            subject.setTeacher((Teacher) school.findPerson("Severus", "Snape").iterator().next());
            schoolClass.addSubject(subject);

            subject = injector.getInstance(Subject.class);
            subject.setName("Black Magic Defense");
            subject.addTerm(new Term(DayOfWeek.FRIDAY, timeFormat.parse("12:30"), 90));
            subject.setTeacher((Teacher) school.findPerson("Dolores", "Umbridge").iterator().next());
            schoolClass.addSubject(subject);
